...
class SMTabbar: UIScrollView {
    ...
    private func estimatedString (_ text :String) -> CGRect {
        ...
        return NSString(string: text).boundingRect(with: size, options: nsstring, attributes: [NSAttributedString.Key.font : DEFAULT_FONT ?? UIFont.systemFont(ofSize: self.fontSize)], context: nil)
    }
